...
if(isset($_POST["action"]))
{
	if($_POST["action"] == "fetch")
	{
		...
		ON class_tbl.class_id = teacher_tbl.teacher_class_id 
		";
		if(isset($_POST["search"]["value"]))
		{
			$query .= '
			WHERE teacher_tbl.teacher_name LIKE "%'.$_POST["search"]["value"].'%" 
			OR teacher_tbl.teacher_emailid LIKE "%'.$_POST["search"]["value"].'%" 
		...
		{
			$query .= '
			ORDER BY teacher_tbl.teacher_id DESC 
			';
		}
		if($_POST["length"] != -1)
		{
		...
		$output = array(
			...
			"recordsFiltered"	=>	get_total_records($connect, 'teacher_tbl'),
			"data"				=>	$data
		);
	...
	if($_POST["action"] == 'Add' || $_POST["action"] == "Edit")
	{
		...
		else
		{
			if($_POST["action"] == 'Add')
			{
				...
				(teacher_name, teacher_address, teacher_emailid, teacher_password, teacher_qualification, teacher_doj, teacher_image, teacher_class_id) 
				SELECT * FROM (SELECT :teacher_name, :teacher_address, :teacher_emailid, :teacher_password, :teacher_qualification, :teacher_doj, :teacher_image, :teacher_class_id) as temp 
				WHERE NOT EXISTS (
					SELECT teacher_emailid FROM teacher_tbl WHERE teacher_emailid = :teacher_emailid
				) LIMIT 1
				";
				$statement = $connect->prepare($query);
	...
	if($_POST["action"] == "single_fetch")
	{
		...
		INNER JOIN class_tbl 
		ON class_tbl.class_id = teacher_tbl.teacher_class_id 
		WHERE teacher_tbl.teacher_id = '".$_POST["teacher_id"]."'";
		$statement = $connect->prepare($query);
	...
	{
		$query = "
		SELECT * FROM teacher_tbl WHERE teacher_id = '".$_POST["teacher_id"]."'
		";
		$statement = $connect->prepare($query);
		if($statement->execute())
		{
			...
			foreach($result as $row)
			{
				...
				$output["teacher_class_id"] = $row["teacher_class_id"];
